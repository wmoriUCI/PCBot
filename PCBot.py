...
async def on_message(message):
    ...
    if message.author.id == 666881033831972884 and message.content[0] == "!":     # This is StressTestBot's ID
        start = time.time()
    ...
    if message_list[0][0] == "!":
        if message_list[0] == "!pc":
            ...
            ret_send = await PCModule.searchItem(item[0], item[1].strip())
            if message.author.id == 666881033831972884:  # This is StressTestBot's ID -> Change this ID
                time_taken = "Time taken to process = " + str(round(time.time() - start, 4)) + "\n"
                content += time_taken
            file_name = await make_text(ret_send, item[0])
            if message.author.id == 666881033831972884:	
